...
const RegulatoryUpdateAlert = ({ updates }) => {
    ...
    return (
        <div className="my-4 p-4 bg-blue-50 border-l-4 border-blue-500 text-blue-800">
            <div className="flex">
                <div className="py-1">
...
const StatusBadge = ({ status }) => {
  ...
  switch (status) {
    case 'Approved': dotColorClass = 'bg-green-500'; textColorClass = 'text-green-700'; break;
    case 'Denied': dotColorClass = 'bg-red-500'; textColorClass = 'text-red-700'; break;
    case 'In Review': case 'Initial Review': case 'Detailed Review': case 'Awaiting Decision':
      dotColorClass = 'bg-yellow-400'; textColorClass = 'text-yellow-700'; break;
    case 'Submitted': dotColorClass = 'bg-blue-400'; textColorClass = 'text-blue-700'; break;
    default: textColorClass = 'text-gray-500'; break;
  }
  return (
...
const FileLink = ({ fileName, href, documentType }) => (
  <div className="py-1">
    <a href={href || '#'} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-700 hover:underline flex items-center group">
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-gray-400 group-hover:text-red-500 transition-colors duration-150 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
      </svg>
      <span className="truncate" title={fileName}>{fileName || 'Unnamed Document'}</span>
      {documentType && <span className="ml-2 text-xs text-gray-400 group-hover:text-gray-600">({documentType})</span>}
    </a>
  </div>
);
...
const InfoRow = ({ label, value, isHtml = false, className = "" }) => (
  <div className={`mb-4 ${className}`}>
    ...
    {isHtml ? (
         <div className="text-sm text-gray-800 break-words" dangerouslySetInnerHTML={{ __html: value || 'N/A' }} />
    ) : (
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50 flex justify-center items-center px-4">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
                <h4 className="text-xl font-semibold mb-1">KYC Check Simulation</h4>
                <p className="text-sm text-gray-600 mb-4">For: <span className="font-medium">{targetName || "Selected Contact"}</span></p>

                {isChecking && (
                    <div className="text-center py-8">
                        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto"></div>
                        <p className="mt-3 text-gray-600">Performing simulated KYC checks...</p>
                ...
                {!isChecking && kycCheckDisplayData && (
                    <div className="space-y-3 text-sm">
                        {kycFields.map(field => (
                            <div key={field.label} className="flex justify-between border-b border-gray-200 py-1.5">
                                <span className="font-medium text-gray-700">{field.label}:</span>
                                <span className="text-gray-600 ml-2 text-right">{field.value || "Data captured from KYC check"}</span>
                            </div>
                        ))}
                    </div>
                )}
...
const ApplicationDetailPage = ({ applicationId, onBackToList }) => {
  ...
  const renderEntityInfo = () => { 
    ...
    return (
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-6">Entity Information</h2>
        ...
        <InfoRow label="Website/URI" value={entity.website ? `<a href="${entity.website.startsWith('http') ? entity.website : `//${entity.website}`}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline">${entity.website}</a>` : 'N/A'} isHtml={!!entity.website}/>
        ...
        )) : <p className="text-sm text-gray-500 py-1">No specific corporate documents found.</p>}

        <SubSectionTitle title="Board Structure / Resolutions" />
        {boardDocs.length > 0 ? boardDocs.map(doc => (
          <FileLink key={doc.documentId} fileName={doc.fileName} href={doc.dummyFileContentLink} documentType={doc.documentType} />
        )) : <p className="text-sm text-gray-500 py-1">No board structure documents found.</p>}
  ...
  const renderProductServiceSolution = () => { 
    if (!product) return <p className="text-gray-500">Product information not available.</p>;
    return (
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-6">Product, Service, or Solution</h2>
    ...
    return (
      <div>
        ...
        {financialDocs.length > 0 ? financialDocs.map(doc => (
          <FileLink key={doc.documentId} fileName={doc.fileName} href={doc.dummyFileContentLink} documentType={doc.documentType} />
        )) : <p className="text-sm text-gray-500 py-1">No financial documents found.</p>}
      </div>
    );
  };
  const renderAmlPlan = () => { 
    ...
    return (
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-6">AML Plan</h2>
        {amlDocs.length > 0 ? amlDocs.map(doc => (
          <FileLink key={doc.documentId} fileName={doc.fileName} href={doc.dummyFileContentLink} documentType={doc.documentType} />
        )) : <p className="text-sm text-gray-500 py-1">No AML Plan documents found.</p>}
      </div>
    );
  };
  const renderCybersecurityPlan = () => { 
    ...
    return (
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-6">Cybersecurity Plan</h2>
        {securityDocs.length > 0 ? securityDocs.map(doc => (
          <FileLink key={doc.documentId} fileName={doc.fileName} href={doc.dummyFileContentLink} documentType={doc.documentType} />
        )) : <p className="text-sm text-gray-500 py-1">No cybersecurity-related documents found.</p>}
      </div>
    );
  };
  const renderGenericSection = (sectionTitle) => { 
    ...
    const relevantDocs = documents.filter(doc => {
     ...
     return (
        <div>
            <h2 className="text-xl font-semibold text-gray-800 mb-6">{sectionTitle}</h2>
            {relevantDocs.length > 0 ? relevantDocs.map(doc => (
                <FileLink key={doc.documentId} fileName={doc.fileName} href={doc.dummyFileContentLink} documentType={doc.documentType}/>
            )) : <p className="text-sm text-gray-500 py-1">No specific documents found for "{sectionTitle}".</p>}
             <p className="text-xs text-gray-400 mt-4 italic">Further details or specific data for this section would be displayed here if available in the application data model.</p>
        </div>
    );
  };
    ...
    return (
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-500 italic text-sm mt-2">(Placeholder for application risk summary)</p>
        </div>
        ...
        {isActionableStatus && (
            <div className="mt-6 pt-6 border-t border-gray-300">
                ...
                <div className="space-y-4 max-w-xl">
                    <div>
                        <label htmlFor="nextStatus" className="block text-sm font-medium text-gray-700">
                    ...
                    {(selectedNextStatus === "Approved" || selectedNextStatus === "Denied" || selectedNextStatus.includes("Clarification") || selectedNextStatus.includes("Deny")) && (
                        <div>
                            <label htmlFor="decisionNotesInput" className="block text-sm font-medium text-gray-700">
                    ...
                    <button 
                        ...
                        className="w-full px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-500 disabled:opacity-50"
                    >
      ...
      <div className="space-y-6">
        ...
        {sortedCommunicationLog.length > 0 ? (
          <div className="space-y-4 max-h-96 overflow-y-auto pr-2"> 
            {sortedCommunicationLog.map(log => (
              <div key={log.logId} className="p-4 bg-gray-50 rounded-lg border border-gray-200 shadow-sm">
                <div className="flex justify-between items-start mb-1">
                  <p className="text-sm font-semibold text-gray-700">{log.type || 'Log Entry'}</p>
                  <p className="text-xs text-gray-500">{formatDate(log.date, true)}</p>
                </div>
                <p className="text-sm text-gray-600 mb-2">{log.summary || 'No summary provided.'}</p>
                <p className="text-xs text-gray-500">Logged by: {log.loggedByStaffId || 'Unknown'}</p>
              </div>
            ))}
          </div>
        ) : <p className="text-sm text-gray-500">No communication log entries for this application.</p>}
        ...
        <div className="mb-4">
            ...
            <button
                ...
                className="mt-2 px-4 py-2 bg-blue-600 text-white font-semibold rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-75 disabled:bg-gray-300 transition-colors text-sm"
            >
                Add Note
            </button>
        </div>
        {sortedGeneralNotes.length > 0 ? (
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2"> 
                {sortedGeneralNotes.map((note, index) => ( 
                    <div key={`note-${index}-${applicationId}`} className="p-3 bg-gray-50 rounded-md border border-gray-200">
                        <p className="text-sm text-gray-700 whitespace-pre-wrap">{note}</p>
                    </div>
                ))}
            </div>
        ) : (
            <p className="text-sm text-gray-500 italic">No general notes added yet.</p>
        )}
      </div>
    );
  };
  ...
  const renderContentForDetailSection = () => {
    switch (activeDetailSection) {
      ...
      default:
        return (
            <div>
                 <h2 className="text-xl font-semibold text-gray-800 mb-6">{activeDetailSection}</h2>
                 <p className="text-gray-500">Content for {activeDetailSection} is not yet implemented or data is unavailable.</p>
            </div>
        );
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex justify-between items-center">
             <h1 className="text-xl font-semibold text-gray-700 truncate">
                License Management &gt; <span className="text-blue-600">{headerData?.companyName || applicationId}</span>
             </h1>
             {onBackToList && (
                <button onClick={onBackToList} className="text-sm text-blue-600 hover:text-blue-800 ml-4 flex-shrink-0 focus:outline-none">
                    &larr; Back to Dashboard
                </button>
             )}
          </div>
      ...
      {headerData && (
        <div className="bg-white border-b border-gray-200 sticky top-[57px] z-10">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-px -mx-px">
                ...
                <div key={item.label} className="py-4 px-2 md:px-3 text-center border-l border-gray-200 first:border-l-0">
                    <p className="text-xs text-gray-500 uppercase tracking-wider truncate" title={item.label}>{item.label}</p>
                    <div className="text-sm font-medium text-gray-800 mt-1 truncate" title={typeof item.value === 'string' ? item.value : (item.label === 'Rating' || item.label === 'Duration' ? String(item.value) : undefined )}>
                        {item.value}
                    </div>
                </div>
                ))}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <nav className="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto" aria-label="Tabs">
            {['LICENSE APPLICATION DETAILS', 'FIT AND PROPER CHECK', 'LICENSE APPLICATION REVIEW', 'COMMENTS/NOTES'].map(tabName => (
              <button
                ...
                className={`${activeMainTab === tabName ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-3 px-2 sm:px-3 border-b-2 font-medium text-sm focus:outline-none`}
                disabled={isProcessingAction}
              >
                {tabName}
              </button>
        ...
        {activeMainTab === 'LICENSE APPLICATION DETAILS' && (
          <div className="flex flex-col md:flex-row gap-6 lg:gap-8">
              ...
              <nav className="space-y-1 md:sticky md:top-[125px]" aria-label="Sidebar"> 
                {detailSections.map(sectionName => (
                  <button
                    ...
                    className={`${activeDetailSection === sectionName ? 'bg-blue-50 border-blue-500 text-blue-700' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-900'} group flex items-center px-3 py-2.5 text-sm font-medium border-l-4 w-full text-left rounded-r-md focus:outline-none transition-colors duration-150`}
                    disabled={isProcessingAction}
                  >
            ...
            <div className="flex-1 bg-white p-6 shadow rounded-lg min-w-0">
                  ...
                  <div className="flex justify-end">
                      {!isLastSection && ( 
                        <button 
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                            disabled={isProcessingAction}
                        >
                            Next
                        </button>
