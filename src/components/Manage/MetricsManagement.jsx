...
import reportingMetricsData from '../../data/reportingMetrics.js'; // Corrected import
...
const MetricsManagement = () => {
  ...
  useEffect(() => {
    setMetrics(reportingMetricsData); // Corrected to use the imported data
  }, []);
  ...
  const handleSaveMetric = (metricData) => {
    if (!metricToEdit) {
      // Create new metric
      const newMetric = { ...metricData, id: `met_${Date.now()}_${Math.random().toString(36).substr(2, 5)}` }; // More unique ID
      setMetrics(prevMetrics => [...prevMetrics, newMetric]);
    } else {
      // Update existing metric
      setMetrics(prevMetrics =>
        prevMetrics.map(metric => (metric.id === metricData.id ? metricData : metric))
      );
    }
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-semibold text-gray-800 mb-4">Metrics Management</h2>
      <p className="text-gray-600 mb-6">
        Define and manage Key Performance Indicators (KPIs) for regulatory operations.
      </p>
      ...
      <div className="bg-white shadow rounded-lg overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {metrics.map((metric) => (
              <tr key={metric.id}>
                <td className="px-6 py-4 whitespace-normal text-sm font-medium text-gray-900 max-w-xs">{metric.name}</td>
                <td className="px-6 py-4 whitespace-normal text-sm text-gray-500 max-w-md">{metric.description}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{metric.calculationSource}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{metric.dataType}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{metric.targetValue}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex items-center space-x-2">
                    <ActionButton
                      icon={<IconView />}
                      label="View Metric Definition"
                      onClick={() => alert(JSON.stringify(metric, null, 2))}
            ...
            {metrics.length === 0 && (
              <tr>
                <td colSpan="6" className="px-6 py-10 text-center text-sm text-gray-500">
                  No metrics defined yet. Click "Define New Metric" to get started.
                </td>
              </tr>
            )}
          </tbody>
